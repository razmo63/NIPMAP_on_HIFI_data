...
ImageID = "7DayRT_31803_slide_13"
SampleID  = "31803_13Tr" 
```

#####  Read the csv data file and filter 7DayRT_31803_slide_13 from the images. Then filter only the Mean intensities of biomarkers. create a column called cell_Id for each uniqe cell and save the new data as .rds file. Don't need to run this chunk everytime. 
...
# prepare the input data for NIPMAP tools according to NIPMAP specifications ("HIFI_7DayRT_TMENsAnalysis.ipynb").
...
cellcol <- c("#f5879a","#FFA500", "#B2DF8A" ,"#33A02C" ,"#CAB2D6","#FFFF99","#E31A1C","#A6CEE3","#000000" ,"#6A3D9A") 
...
#####  Find coordinates of a big rectangle that is 100% within the brain+tumor tissue, due to the fact that NIPMAP only supports rectangular datasets. Then write the data as a CSV file as input for "HIFI_7DayRT_TMENsAnalysis.ipynb" . The data is now prepared for NIPMAP tools, and the chunks in "HIFI_7DayRT_TMENsAnalysis.ipynb" need to be executed. The remaining analysis would then be performed here, beginning with "NICH PHENOTYPE ASSOCIATIONS."

```{r,croped rectangular, fig.align='center',echo= TRUE}


cellPosType <- filter(cellPosType ,cellPosType$x>2400 & cellPosType$x<5500 & cellPosType$y>1750 &cellPosType$y<3200 )
...
### load json objects which was saved in "HIFI_7DayRT_TMENsAnalysis.ipynb" 
...
### Niche-phenotype mapping  was not apply on treatment group (7daysIR)
